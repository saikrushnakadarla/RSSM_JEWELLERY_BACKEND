...
router.post("/delivery-agents/add", (req, res) => {
  ...
  if (!name || !email || !mobile || !vendorid || !da_password) {
    return res
      .status(400)
      .json({ error: "All fields, including vendor ID, are required" });
  }

  const sql =
    "INSERT INTO delivery_agents (name, email, mobile, vendorid,da_password) VALUES (?, ?, ?, ?,?)";
  db.query(sql, [name, email, mobile, vendorid, da_password], (err, result) => {
    ...
    res
      .status(201)
      .json({ id: result.insertId, name, email, mobile, vendorid });
  });
});
...
router.put("/delivery-agents/update/:id", (req, res) => {
  ...
  const sql =
    "UPDATE delivery_agents SET name=?, email=?, mobile=?, vendorid=?  WHERE id=?";
